...
impl LogicalTimeBarrier {
    ...
    pub fn acquire_tag(
        ...
        tracing::trace!("Releasing provisional tag");
...
impl Scheduler {
    ...
    pub fn startup(&mut self) {
        ...
        self.release_tag(self.current_tag);
    ...
    fn release_tag(&self, current_tag: Tag) {
        for (key, ctx) in self.downstream_enclaves.iter() {
            tracing::trace!(downstream = %key, "Releasing downstream");
            if !ctx.schedule_async(AsyncEvent::TagRelease {
                enclave: self.key,
                tag: current_tag,
            }) && self.shutdown_tag.is_none()
            {
                tracing::warn!(
    ...
    #[tracing::instrument(skip(self), fields(self.current_tag))]
