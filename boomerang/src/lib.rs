...
struct Scheduler<T: EventValue> {
    ...
    // Priority queues.
    /// For sorting by time.
    event_q: BTreeSet<Box<Event<T>>>,
    /// For sorting by deadline
    reaction_q: BTreeSet<Rc<Reaction<T>>>,
    /// For recycling malloc'd events.
    recycle_q: Vec<Box<Event<T>>>,
    /// For free malloc'd values carried by events.
    free_q: Vec<Box<Event<T>>>,
}

impl<T: EventValue> Scheduler<T> {
    fn new() -> Scheduler<T> {
        Scheduler {
            fast: false,
            ...
            free_q: Vec::with_capacity(INITIAL_EVENT_QUEUE_SIZE),
        }
    }
    ...
    fn find_event_bounded(&self, event: &Box<Event<T>>, limit: &Instant) -> Option<&Box<Event<T>>> {
        let rng = self.event_q.range::<Box<Event<T>>, _>(event..);
        let found = rng.rev().find(|&x| {
            println!("ev: {:?}", x.time);
            x.trigger == event.trigger || x.time == *limit
        });
    ...
    fn schedule_output_reactions(&mut self, reaction: Rc<Reaction<T>>) {
    ...
    fn next(&mut self) -> bool {
        ...
        if !self.wait_until(next_time) {
            ...
            if new_event.filter(|ev| ev.trigger == event_trigger).is_some() {
        ...
        loop {
            ...
            {
                let mut trigger = event.trigger.borrow_mut();
                ...
                trigger.value = event.value.clone();
            }
...
mod tests {
    ...
    */
    ...
    #[test]
    fn test() {
        // let trig = Rc::new(Trigger {
        // offset: std::time::Duration::from_micros(0),
        // period: std::time::Duration::from_micros(0),
        ...
        // });
        //
        // let trig2 = Rc::new(Trigger {
        // offset: std::time::Duration::from_micros(100),
        // period: std::time::Duration::from_micros(100),
        // policy: QueuingPolicy::NONE,
        // is_physical: false,
        // reactions: vec![],
        // scheduled: None,
        ...
        //
        // let mut event_q = BTreeSet::<Rc<Event>>::new();
        //
        // for i in 0..5 {
        // let ttx = tt + std::time::Duration::from_nanos(100 * i);
        // event_q.insert(Rc::new(Event::new(ttx, trig.clone())));
        // }
        //
        // let ev150 = Rc::new(Event::new(
        // tt + std::time::Duration::from_nanos(150),
        // trig2.clone(),
        // ));
        //
        // let ev300 = Rc::new(Event::new(
        // tt + std::time::Duration::from_nanos(300),
        // trig.clone(),
        // ));
        // event_q.insert(ev150.clone());
        //
        // let x = event_q.pop_last().unwrap();
        // println!("x: {:?}", x.time - tt);
        // let x = event_q.pop_last().unwrap();
        // println!("x: {:?}", x.time - tt);
        //
        // println!("{:?}", rng.next().unwrap().time - tt);
    }

    #[test]
    fn it_works() {
        // let mut r = Box::new(Reaction {
        // function: |s: &mut TestReactor| {
        // println!("Hello World!");
        // },
        // state: TestReactor { i: 0 },
        // index: 0,
        // chain_id: 1,
        // pos: 0,
        // num_outputs: 1,
        //
        // output_produced: helloworldtest_a_reaction_0_outputs_are_present,
        // &helloworldtest_a_reaction_0_triggered_sizes[0],
        // &helloworldtest_a_reaction_0_triggers[0],
        // triggers: vec![vec![]],
        // running: false,
        // local_deadline: 0,
        ...
        // });
        //
        let foo = Rc::new(RefCell::new(Trigger {
            reactions: vec![],
            offset: Duration::from_nanos(0),
            period: Some(Duration::from_nanos(1000)),
            value: Rc::new(RefCell::new(None)),
            is_physical: true,
            scheduled: None, // NEVER
            policy: QueuingPolicy::UPDATE,
        }));

        let bar = Rc::new(RefCell::new(Trigger {
            reactions: vec![],
            offset: Duration::from_nanos(100),
            period: Some(Duration::from_nanos(1000)),
            value: Rc::new(RefCell::new(None)),
            is_physical: true,
            scheduled: None, // NEVER
        ...
        scheduler.schedule(foo, Duration::from_nanos(0), Some("foo"));
        scheduler.schedule(bar, Duration::from_nanos(0), Some("bar"));

        //(r.function)(&mut r.state);
        scheduler.print_event_queue();
        scheduler.print_reaction_queue();
