...
The current in-memory runner executes persistent static federates with the same
logical-time scheduler hooks used by the protocol client. A typical setup
registers a codec, builds runtime parts, and then calls the federated runner:
...
behavior, authentication, direct federate-to-federate payload channels, or a
scheduler-running TCP static runner yet.
