...
pub trait ActionData: Debug + Send + Sync + Downcast {}

#[cfg(not(feature = "serde"))]
impl<T> ActionData for T where T: Debug + Send + Sync + Downcast {}
...
pub trait BaseAction: Debug + Send + Sync + Downcast {
    ...
    fn push_value(&mut self, tag: Tag, value: Box<dyn ActionData>);
}
...
#[derive(Debug)]
pub struct Action<T: ActionData = ()> {
    name: String,
    key: ActionKey,
...
impl<T: ActionData> BaseAction for Action<T> {
    fn name(&self) -> &str {
        &self.name
    }
