...
- [x] (2026-07-09 07:39Z) Refactored the Milestone 4 builder API so `connect_federated_port` was removed, `EnvBuilder::register_federated_codec::<T, _>(...)` registers payload codec policy by type, and ordinary `connect_port` infers cross-federate endpoint lowering from partition placement.
- [x] (2026-07-09 07:39Z) Added focused regression coverage that local cross-enclave connections do not require federated codecs, missing cross-federate codecs produce a clear type-specific build error, and cross-federate examples use codec registration plus `connect_port`.
- [x] (2026-07-09 07:39Z) Added in-memory distributed execution tests for startup hello and delayed connection tag equivalence. The harness runs source and sink federate enclaves separately, routes outbound MSG commands through `boomerang_federated::RtiState`, schedules delivered RTI MSG frames through the inbound endpoint registry, and compares the recorded values/tags with the local enclave oracle.
- [x] (2026-07-09 07:39Z) Kept the distributed zero-delay-cycle rejection test on ordinary `connect_port` and registered the payload codec in that test so the observed failure remains the explicit unsupported-topology error.
- [x] (2026-07-09 07:57Z) Started Milestone 6 implementation, limited to a length-delimited TCP `ProtocolFrame` transport and ignored localhost smoke coverage inside `boomerang_federated`. The Boomerang reaction scheduler remains synchronous, `boomerang_runtime` remains independent of `boomerang_federated`, and runtime outbound/inbound endpoint behavior continues to be exercised by the existing in-memory builder tests.
- [x] (2026-07-09 08:00Z) Added `TcpTransport`, exported it from `boomerang_federated`, and added an ignored localhost smoke test that starts an RTI listener plus source and sink TCP clients, sends one `MSG` payload through `RtiState`, delivers it to the sink, exchanges `Stop`, and shuts down cleanly.
- [x] (2026-07-09 08:00Z) Ran the requested targeted validation: `cargo test -p boomerang_runtime`, `cargo test -p boomerang_runtime --features federated`, `cargo test -p boomerang_federated`, `cargo test -p boomerang_builder --features federated`, and `cargo check -p boomerang --features federated --benches` all passed. Also ran `cargo test -p boomerang_federated tcp_smoke -- --ignored`; the sandboxed run was denied localhost bind permission, and the escalated localhost run passed.
- [x] (2026-07-09 08:29Z) Replaced the hand-rolled TCP frame read/write logic with `tokio_util::codec::LengthDelimitedCodec` while preserving the `TcpTransport` API, serde JSON `ProtocolFrame` encoding, and the ignored localhost smoke test behavior.
- [x] (2026-07-09 08:29Z) Validated the codec-backed transport with `cargo test -p boomerang_federated`, `cargo check -p boomerang --features federated --benches`, `cargo test -p boomerang_federated tcp_smoke -- --ignored`, and `cargo test -p boomerang_builder --features federated`; all passed.
...
- Observation: The runtime endpoint support from Milestone 4 was enough to write deterministic in-memory distributed execution tests without adding a network client or a `boomerang_runtime` dependency on `boomerang_federated`.
  Evidence: `cargo test -p boomerang_builder --features federated` now includes `test_in_memory_distributed_hello_matches_local_enclave` and `test_in_memory_distributed_delayed_connection_matches_local_tag`, both of which drain `FederatedOutboundBuffer`, route MSG through `RtiState`, schedule through `FederatedInboundEndpointRegistry`, and compare against local enclave execution.

- Observation: The managed sandbox denies binding even a localhost ephemeral TCP listener, but the ignored smoke test passes when that one command is allowed to bind locally.
  Evidence: `cargo test -p boomerang_federated tcp_smoke -- --ignored` first failed at `TcpListener::bind("127.0.0.1:0")` with `Operation not permitted`; rerunning the same command with escalation passed with 1 test.
...
- Decision: Remove `connect_federated_port` instead of keeping it as a public override in this slice.
  Rationale: The corrected model has one semantic graph API, `connect_port`. Cross-federate behavior is a lowering consequence of `ReactorPlacement` plus partitioning, and codec selection is environment policy registered by payload type. Keeping a public per-edge method would preserve the confusion the correction was meant to remove. A future explicit override can be added with a concrete use case and tests.
  Date/Author: 2026-07-09 / Codex

- Decision: Implement Milestone 6 TCP support as a `boomerang_federated` `TcpTransport` for serde JSON encoded `ProtocolFrame`s, length-delimited with `tokio_util::codec::LengthDelimitedCodec`.
  Rationale: `ProtocolFrame` is the existing protocol envelope and serde JSON is already the first payload/protocol codec in the MVP. `LengthDelimitedCodec` provides the same reliable ordered frame boundary shape as the original manual big-endian `u32` prefix, but delegates partial reads, EOF handling, and maximum-frame checks to a maintained Tokio utility. Tokio and `tokio-util` stay scoped to `boomerang_federated`; no Tokio path is added for Boomerang reaction scheduling and no dependency from `boomerang_runtime` to `boomerang_federated` is introduced.
  Date/Author: 2026-07-09 / Codex
...
Milestone 4 serialized endpoint lowering is implemented with the corrected builder API. `boomerang_runtime` exposes feature-gated runtime-owned endpoint ids, payload encoder/decoder traits, an outbound MSG command buffer, an inbound endpoint registry, and `FederatedSenderReactionFn<T>`. These are in-process integration interfaces, not wire serialization; a later federated client remains responsible for converting runtime tags to `WireTag`. The builder now keeps ordinary `connect_port` as the graph API, infers cross-federate lowering from `ReactorPlacement` and partitioning, and resolves payload codecs from an `EnvBuilder` registry keyed by payload type. Same-partition and local cross-enclave paths remain unchanged, and missing codecs fail only for inferred cross-federate edges with a type-specific build error. Focused builder tests cover codec rejection, endpoint lowering without local crosslinks, outbound delayed MSG command emission, inbound registry scheduling, local cross-enclave non-serialized payload behavior, and zero-delay-cycle rejection.

Milestone 5 in-memory distributed execution coverage is implemented for the static MVP. The new tests build source and sink as separate federates, run the source enclave separately, drain its serialized outbound MSG command, route that MSG through `boomerang_federated::RtiState`, schedule the RTI-delivered payload through the destination `FederatedInboundEndpointRegistry`, then run the sink enclave and compare the recorded value/tag against an equivalent local enclave run. The hello case observes `(Tag::ZERO, 7)`. The delayed case observes `Tag::new(Duration::milliseconds(10), 0)` in both local and federated runs. Distributed zero-delay cycles remain rejected at build time. No TCP networking, process launch, PTAG/ABS support, transient federates, authentication, or full federated runner/client was added.

Milestone 6 TCP smoke coverage is implemented in `boomerang_federated`. The crate now exposes `TcpTransport`, a Tokio-backed transport that sends serde JSON encoded `ProtocolFrame` values over TCP using `tokio_util::codec::LengthDelimitedCodec` and a configurable maximum frame size. The ignored smoke test binds a localhost RTI listener, accepts source and sink federate clients, receives their `Hello` frames, sends `Start`, routes one source `MSG` through `RtiState` to the sink, then exchanges `Stop` frames and joins all tasks cleanly. This slice does not add a federated process runner, does not add a Tokio scheduler path for Boomerang reactions, and does not make `boomerang_runtime` depend on `boomerang_federated`.
...
Change note: The plan was updated on 2026-07-09 after correcting the Milestone 4 builder API and implementing Milestone 5 in-memory distributed tests. `connect_federated_port` was removed, `EnvBuilder::register_federated_codec` became the type-keyed codec registration API, ordinary `connect_port` now infers cross-federate endpoint lowering, and the new in-memory tests route MSG frames through `RtiState` before scheduling destination endpoint payloads.
