...
use super::{Action, ActionCommon, ActionData, ActionKey, BaseAction};
...
pub struct ActionRef<'a, T: ActionData = ()>(&'a mut Action<T>);

impl<'a, T: ActionData> From<&'a mut dyn BaseAction> for ActionRef<'a, T> {
    fn from(value: &'a mut dyn BaseAction) -> Self {
        Self(value.downcast_mut().expect("Type mismatch on ActionRef2"))
    }
}

impl<'a, T: ActionData> ActionRef<'a, T> {
...
impl<'a, T: ActionData> ActionCommon for ActionRef<'a, T> {
    fn name(&self) -> &str {
        self.0.name()
    }
...
impl<'a, T: ActionData> From<&'a mut dyn BaseAction> for AsyncActionRef<T> {
    fn from(value: &'a mut dyn BaseAction) -> Self {
...
impl<T: ActionData> AsyncActionRef<T> {
    ...
    pub fn schedule(&self, context: &SendContext, value: T, delay: Option<Duration>) {
        ...
        let value = Box::new(value) as Box<dyn ActionData>;
...
impl<T: ActionData> ActionCommon for AsyncActionRef<T> {
    fn name(&self) -> &str {
        &self.name
    }
