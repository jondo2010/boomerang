...
The constructors declare actions, ports, reactions, and timing directly in the
compiler topology. They do not construct game state, initialize the terminal, or
invoke legacy `Assembly` builders. The shared enclave is explicit, and connections
require matching payload types and output-to-input direction.

Generated executables use tool-owned `boomerang_facet="descriptor"` and
...
The public topology constructors are tested through ordinary Cargo dependencies:
