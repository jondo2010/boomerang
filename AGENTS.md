...
Federated code has a strict crate boundary. `boomerang_runtime` owns protocol-free scheduler/runtime hooks and must not depend on `boomerang_federated`, Tokio protocol code, or RTI orchestration. `boomerang_federated` owns wire protocol, RTI state/session logic, federate clients, runtime protocol bridges, and static/TCP federation orchestration behind its `runtime` feature. `boomerang_builder` owns builder metadata, topology validation, endpoint lowering, and thin builder-facing shims that translate `BuilderRuntimeParts` into runtime/protocol DTOs. `boomerang` should only re-export public APIs.
