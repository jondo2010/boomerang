    ...
    pub(crate) fn rebase_action_values(
        ...
        if let Some(mut moves) = moves {
            ...
            for (action_key, from, to) in moves {
                ...
                if from != to {
                    tracing::trace!(target: crate::trace::TRACE_TARGET, event = crate::trace::TraceEvent::ActionSchedule as u64, action_key = %action_key, old_logical_ns = crate::trace::logical_ns(from), old_microstep = crate::trace::microstep(from), destination_logical_ns = crate::trace::logical_ns(to), destination_microstep = crate::trace::microstep(to), outcome = crate::trace::TraceOutcome::Rebased as u64);
                }
            }
