...
fn baseline_messages_have_canonical_vectors_and_borrow_payloads() {
    ...
    let cases = [
        ...
        (
            ...
            [&[2, 1][..], &tag_bytes, &[1, 42]].concat(),
        ),
        (Message::ConfirmIdle { revision: 300 }, vec![3, 0xac, 2]),
        (Message::Stop, vec![4]),
        (Message::Abort { message: "x" }, vec![5, 1, b'x']),
        (Message::Started, vec![6]),
        (
            Message::Grant { revision: 300, tag },
            [&[7, 0xac, 2][..], &tag_bytes].concat(),
        ),
        (
            ...
            [&[8, 1][..], &tag_bytes, &[1, 42]].concat(),
        ),
        (Message::Idle { revision: 300 }, vec![9, 0xac, 2]),
        (Message::Stopped, vec![10]),
        (Message::Failed { message: "x" }, vec![11, 1, b'x']),
    ];
    for kind in [0, 2] {
        let mut s = Session::new(&c, Member::new(0)).unwrap();
        preflight(&mut s, "alpha");
        let encoded = traffic(&[1, kind]);
...
fn handshake_vector_and_every_identity_mismatch_fail_closed() {
    ...
    assert_eq!(early.decode(&traffic(&[6])), Err(WireError::NotAdmitted));
    assert_eq!(
...
fn malformed_frames_cannot_admit_routes_or_survive_failure() {
    ...
    for kind in [12, 13, 255] {
        bad_frames.push(traffic(&[kind]));
    }
    for offset in [4, 5, 6, 7, 9, 10, 13] {
    ...
    for bytes in bad_frames {
        ...
        assert_eq!(s.decode(&traffic(&[6])), Err(WireError::SessionFailed));
    }
...
fn bounds_are_checked_without_size_driven_storage() {
    ...
    let mut s = Session::new(&c, Member::new(0)).unwrap();
    preflight(&mut s, "alpha");
    let oversized = traffic(&[&[2, 0, 0, 0x80, 0x80, 4][..], &[0; MAX_PAYLOAD_BYTES + 1]].concat());
    assert_eq!(
        s.decode(&oversized),
    ...
    preflight(&mut s, "alpha");
    assert_eq!(
        s.encode(&Message::Started, &mut [0; 8]),
    ...
    let largest = Message::PayloadToRti {
        route: u32::MAX,
...
fn invalid_rosters_and_terminal_failure_cannot_reopen_admission() {
    ...
    for kind in [5, 11] {
        let mut s = Session::new(&c, Member::new(0)).unwrap();
        preflight(&mut s, "alpha");
        assert!(s.decode(&traffic(&[kind, 0])).is_ok());
        assert_eq!(s.decode(&traffic(&[6])), Err(WireError::SessionFailed));
    }
    for bytes in [
        traffic(&[5, 1, 255]),     // Invalid UTF-8.
        traffic(&[0, 0, 2]),       // Invalid Option discriminant.
        traffic(&[0, 0x80, 0, 0]), // Overlong revision.
        traffic(&[0x86, 0]),       // Overlong enum discriminant.
        traffic(&[5, 0x80, 0]),    // Overlong string length.
        traffic(&[1, 0, 0]),       // Trailing bytes after Never.
    ] {
        let mut s = Session::new(&c, Member::new(0)).unwrap();
        preflight(&mut s, "alpha");
        assert!(s.decode(&bytes).is_err(), "accepted {bytes:?}");
