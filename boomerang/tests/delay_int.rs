...
fn Delay(
    ...
    let d = ctx.add_logical_action::<u32>("d", None)?;
...
fn DelayInt() -> impl Reactor<(), Ports = DelayIntPorts> {
    let t = ctx.get_startup_action();
    let d = ctx.add_child_reactor(Delay(Duration::milliseconds(100)), "d", (), false)?;
    let test = ctx.add_child_reactor(Test(), "test", TestState::default(), false)?;
    ctx.connect_port(d.out, test.in_, None, false)?;
    ctx.add_reaction(None)
        .with_trigger(t)
        .with_effect(d.in_)
