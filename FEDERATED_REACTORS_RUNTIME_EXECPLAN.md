...
- Observation: In this Milestone 5 session, CodeStory local navigation was fresh for `/Users/johhug01/Source/boomerang`, but packet/search stayed blocked by stale sidecar retrieval even after the MCP auto-repair and `repair_all` paths.
...
- Observation: The existing `boomerang_federated/runtime` feature was not enough for a scheduler barrier bridge because runtime federated endpoint and barrier types are behind `boomerang_runtime/federated`.
  Evidence: `cargo test -p boomerang_federated --features runtime` initially failed with configured-out `boomerang_runtime::FederatedEndpointId`, `FederatedOutboundReceiver`, `FederatedInboundEndpointRegistry`, `AsyncEvent`, and `FederatedTimeBarrier`; changing the feature to include `boomerang_runtime/federated` fixed the compile.

- Observation: The bridge methods are synchronous and will block the current thread while waiting for protocol frames, so tests that also drive fake RTI tasks need a multi-thread Tokio runtime.
  Evidence: the first `cargo test -p boomerang_federated --features runtime` hung with all three `client::tests::*` still running after 60 seconds under the default current-thread Tokio test runtime; changing those tests to `#[tokio::test(flavor = "multi_thread", worker_threads = 2)]` let them pass.

- Observation: The in-memory runner must connect all federate protocol clients concurrently.
  Evidence: the first Milestone 6 runner attempt connected one federate and waited for its `Start`, while `StaticRtiSession` waits for every federate's `Hello` before sending any `Start`; both live builder tests hung until the runner spawned all `FederateProtocolClient::connect` futures on the Tokio runtime before awaiting them.

- Observation: An interrupted barrier wait can later observe a grant for the originally requested future tag while waiting for an earlier inbound tag.
  Evidence: in the delayed source/sink test, the sink first requested its timeout tag, was interrupted by an inbound `MSG(10ms)`, and later received the queued `TAG(100ms)` while waiting for `TAG(10ms)`. Treating a grant greater than the current request as sufficient fixed the live delayed test because the later grant proves the earlier tag is safe.

- Observation: Milestone 6 can close the RTI session with explicit post-scheduler `Stop` frames without implementing Milestone 7 no-future semantics.
  Evidence: `RtiFederatedTimeBarrier::stop_result` is called by the builder runner after scheduler threads terminate; the live tests still rely on ordinary future timeout events to advertise a later `NET`, and the runner still rejects no-reaction/no-future federate enclaves as reserved for a later milestone.

- Observation: The live in-memory runner is runtime orchestration, not builder logic, but it still cannot move into `boomerang_runtime`.
  Evidence: `boomerang_federated/src/static_runner.rs` now owns Tokio runtime construction, `StaticRtiSession`, `FederateProtocolClient`, in-memory protocol transports, and scheduler barrier threads. `boomerang_builder/src/federation.rs` now only validates builder-specific topology constraints, maps builder federates to runtime enclaves, and lowers to `boomerang_federated::StaticFederationRuntimeParts`.
...
- Decision: Put the Milestone 5 client bridge in `boomerang_federated/src/client.rs` behind the existing `runtime` feature, and make that feature enable `boomerang_runtime/federated`.
  Rationale: The bridge necessarily mentions both protocol types and runtime scheduler/endpoint types, so it belongs outside `boomerang_runtime`. Enabling `boomerang_runtime/federated` only when `boomerang_federated/runtime` is requested exposes the runtime barrier and endpoint APIs without adding a dependency from `boomerang_runtime` back to `boomerang_federated`.
  Date/Author: 2026-07-09 / Codex

- Decision: Split the client bridge into an async protocol client and a synchronous scheduler barrier.
  Rationale: `FederateProtocolClient::connect` performs the protocol `Hello`/`Start` handshake and spawns async reader/writer tasks over the existing `Sink<ProtocolFrame>` and `TryStream<Ok = ProtocolFrame>` transports. `RtiFederatedTimeBarrier` then implements the synchronous `boomerang_runtime::FederatedTimeBarrier` trait by sending `NET`, draining runtime outbound commands into `MSG`, sending `LTC`, and scheduling inbound `MSG` frames through `FederatedInboundEndpointRegistry`.
  Date/Author: 2026-07-09 / Codex

- Decision: Acknowledge an inbound protocol `MSG` with an immediate `LTC` for the message tag after scheduling it through the runtime inbound endpoint registry.
  Rationale: The current `RtiState` keeps forwarded messages in its in-transit table until the target reports `LTC` at or beyond the message tag. Sending this delivery acknowledgment unblocks pending grants while the scheduler still receives and processes the queued async event; the later scheduler completion `LTC` remains idempotent. This preserves the current RTI accounting without implementing `PTAG`, `ABS`, or distributed zero-delay cycle support.
  Date/Author: 2026-07-09 / Codex

- Decision: Initially implement the Milestone 6 public runner in `boomerang_builder::execute_federation_in_memory` behind the existing `federated` feature. This decision was later narrowed by the `boomerang_federated::static_runner` refactor below.
  Rationale: The runner consumes `BuilderRuntimeParts`, so builder-owned code was the first practical place to validate federation plans, map federates to runtime enclaves, install live outbound endpoint routes, build in-memory protocol transports, and return `TinySecondaryMap<EnclaveKey, Env>` like local execution. Protocol state and Tokio tasks remained outside `boomerang_runtime`, and `runtime::execute_enclaves` remained unchanged.
  Date/Author: 2026-07-09 / Codex

- Decision: Preserve `FederatedOutboundBuffer` while adding a live outbound sink indirection. This decision was later refined by moving the sink from builder to `boomerang_runtime::BufferedFederatedOutboundSink`.
  Rationale: Existing manual tests still need deterministic buffer drains, while the live runner needs per-federate waking channels. The live sink records every command in the existing buffer and, when the runner installs endpoint routes, forwards matching commands to a `FederatedOutboundChannel` for the source federate's protocol barrier.
  Date/Author: 2026-07-09 / Codex

- Decision: Move Milestone 6 runtime orchestration from `boomerang_builder` to `boomerang_federated::static_runner`, keeping a builder-facing shim.
  Rationale: The execution loop depends on protocol clients, RTI sessions, in-memory protocol transports, Tokio, and scheduler barriers, so it belongs with the federated runtime integration layer. The builder still owns `FederationPlan` and alias lowering because those depend on builder keys and FQNs. This preserves the public `boomerang_builder::execute_federation_in_memory` API without adding a dependency from `boomerang_runtime` to `boomerang_federated`.
  Date/Author: 2026-07-09 / Codex

- Decision: Replace `BuilderFederatedOutboundSink` with `boomerang_runtime::BufferedFederatedOutboundSink`.
...
Milestone 5 is complete. `boomerang_federated/src/client.rs` now defines `FederateProtocolClient`, `FederateClientRoute`, `FederateClientError`, and `RtiFederatedTimeBarrier` behind the `boomerang_federated/runtime` feature. `FederateProtocolClient::connect` sends `Hello`, waits for `Start`, and then drives reader/writer tasks over the same `Sink<ProtocolFrame>` and `TryStream<Ok = ProtocolFrame>` transport shape used by the RTI session. `RtiFederatedTimeBarrier` owns one protocol client, one `FederateId`, one runtime `FederatedOutboundReceiver`, one runtime `FederatedInboundEndpointRegistry`, and endpoint route metadata. Its `acquire_tag_result` and `FederatedTimeBarrier::acquire_tag` path sends `NET`, waits for a matching `TAG`, handles `MSG` by scheduling the payload through the inbound endpoint registry and returning the queued `AsyncEvent`, and records `Stop`/`Error` frames as typed errors. Its `logical_tag_complete_result` and `FederatedTimeBarrier::logical_tag_complete` path drains outbound runtime commands into protocol `MSG` frames and sends `LTC`. The bridge does not construct schedulers, expose a static runner, add TCP orchestration, replace builder manual tests, implement no-future shutdown semantics, or implement `PTAG`/`ABS`. Validation passed with the requested commands: `cargo test -p boomerang_federated`, `cargo test -p boomerang_builder --features federated`, `cargo test -p boomerang_runtime --features federated`, and `cargo check -p boomerang --features federated`. An additional focused check, `cargo test -p boomerang_federated --features runtime`, passed with `22 passed; 0 failed; 1 ignored`.
...
Change note: Revised on 2026-07-09 to complete Milestone 5 by adding the runtime-gated federate client bridge in `boomerang_federated/src/client.rs`. The revision records the async protocol client plus synchronous scheduler barrier split, the `boomerang_federated/runtime` feature dependency on `boomerang_runtime/federated`, and the current inbound `MSG` delivery acknowledgment behavior required by `RtiState` in-transit accounting.

Change note: Revised on 2026-07-09 to complete Milestone 6 by adding `boomerang_builder::execute_federation_in_memory`, the live outbound sink indirection, concurrent in-memory federate client startup, post-scheduler Stop cleanup, and live builder tests for zero-delay and delayed source/sink delivery.

Change note: Revised on 2026-07-09 to move Milestone 6 runtime orchestration into `boomerang_federated::static_runner` and replace `BuilderFederatedOutboundSink` with `boomerang_runtime::BufferedFederatedOutboundSink`, leaving `boomerang_builder::execute_federation_in_memory` as a builder metadata lowering shim.
