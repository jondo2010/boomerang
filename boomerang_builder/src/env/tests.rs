...
fn test_reactions1() {
    ...
    let (reactor_key1, reaction_keys) = builder_closure("test_reactor", &mut env_builder);
    ...
    let (env, _aliases) = env_builder.build_runtime(reactor_key1).unwrap();
    ...
    let (env, _aliases) = env_builder.build_runtime(reactor_key2).unwrap();
    assert_eq!(env.reactions.len(), 2);
}
...
pub mod test_reactor {
    ...
    impl Reactor for ABuilder {
        ...
        fn build(
            ...
            let o = __builder.add_port::<()>("o", PortType::Output)?;
            let reactor = Self { o };
    ...
    impl Reactor for BBuilder {
        ...
        fn build(
            ...
            let mut __builder = env.add_reactor(name, parent, state);
            let i = __builder.add_port::<()>("i", PortType::Input)?;
            let reactor = Self { i };
            Ok((__builder.finish()?, reactor))
        }
    }
    ...
    impl Reactor for CBuilder {
        ...
        fn build(
            ...
            let __b_state = ();
            let mut __builder = env.add_reactor(name, parent, state);
            let (_key, a): (BuilderReactorKey, ABuilder) =
                __builder.add_child_reactor("a", __a_state)?;
            let (_key, b): (BuilderReactorKey, BBuilder) =
